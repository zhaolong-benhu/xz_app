...
export default class VideoPlayer extends Component {
	...
	constructor( props ) {
		...
		this.methods = {
			...
			togglePlayPause: this._togglePlayPause.bind( this,false ),
	...
	_onError( err ) {
		if(this.props.onError) {
			console.log('======>8888',this.state.paused)
			//播放器暂停处理
			err.error.extra=='-20' ? this._togglePlayPause(true) : this.props.onError(err)
		}
		let state = this.state;
		state.error = true;
		state.loading = false;
	 ...
	 */
	_onScreenPress() {
		if(this.props.onScreenPress) {
			this.props.onScreenPress()
		}
		let state = this.state;
	 ...
	 */
	_togglePlayPause(paused) {
		let state = this.state;
		state.paused = paused ? paused : ! state.paused;
		this.setState( state );
	}
	...
	componentDidMount() {
		...
		// this.seekTo( parseInt(this.props.seekerPosition || 0) );
		// this.setState({
		// 	currentTime: 1000
		// })
		// this.player.ref.seek( 1000 )
	}
	...
	renderSeekbar() {
		return (
			<View style={{width:300,height:16,justifyContent:'center'}}>
				<View
